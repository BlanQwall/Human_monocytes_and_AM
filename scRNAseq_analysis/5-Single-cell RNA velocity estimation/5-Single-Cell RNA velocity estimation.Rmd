...
seurat.combined <- readRDS(file = "../../../Human_monocytes_and_AM/scRNAseq_analysis/3-Merge and cell typing/so.merged_clusters.seuratObject.Rds")
```
...
Remove cells that are not in Seurat object. 
...
obj.sl <- obj$`Healthy non-smokers`
pfile <- Convert.seurat_loom(from = obj.sl, to = "loom", filename = "No_Fumer.loom" )
pfile$close_all()
```

For "Non-COPD smokers":

```{r eval=FALSE}
obj.sl <- obj$`Non-COPD smokers`
...
```
![scVelo results for "Healthy non-smokers" grouped sample](Results/scvelo_No_Fumer.png)
...
```
![scVelo results for "Non-COPD smokers" grouped sample](Results/scvelo_Fumer.png)
...
```
![scVelo results for "COPD smokers" grouped sample](Results/scvelo_COPD.png)
